...
export default function CustomerSessionWorkspace({ dogName, plan, loading, onBack, onTalkToCoach, onSaveSession }: CustomerSessionWorkspaceProps) {
  ...
  const completeSession = async () => {
    setSaving(true);
    try {
      setReview(await onSaveSession({
        rating,
        wins,
        difficult,
        notes,
        completedSteps: completedSteps.map((step) => step.replace(/^\d+-/, "")),
      }));
    } finally {
      setSaving(false);
    }
