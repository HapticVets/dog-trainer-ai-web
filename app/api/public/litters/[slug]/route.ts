...
export async function GET(_request: Request, { params }: Context) {
  ...
  const { data: puppies, error: puppiesError } = await supabaseAdmin.from("admin_litter_puppies").select("id,collar_color,sex,public_name,public_summary,public_status,public_price,public_color").eq("litter_id", litter.id).eq("is_public", true).order("created_at");
  ...
  for (const item of media ?? []) { const { data } = await supabaseAdmin.storage.from(mediaBucket).createSignedUrl(item.storage_path, 60 * 60); if (!data?.signedUrl) continue; const entries = mediaByPuppy.get(item.puppy_id) ?? []; entries.push({ type: item.media_type, url: data.signedUrl, caption: item.public_caption || null, isPrimary: item.is_public_primary, capturedAt: item.captured_at }); mediaByPuppy.set(item.puppy_id, entries); }
  const summariesByPuppy = new Map<string, Array<{ week: number; summary: string; createdAt: string; updatedAt: string }>>();
  for (const item of summaries ?? []) { const entries = summariesByPuppy.get(item.puppy_id) ?? []; entries.push({ week: item.development_week, summary: item.summary, createdAt: item.created_at, updatedAt: item.updated_at }); summariesByPuppy.set(item.puppy_id, entries); }
  ...
  return NextResponse.json({ litter: { slug: publicSlug, title: litter.public_title || litter.name, summary: litter.public_summary || null, status: litter.public_status || null, birthDate: litter.birth_date, expectedGoHomeDate: litter.expected_go_home_date, updatedAt: litter.public_updated_at }, sire: litter.sire_dog_id ? dogsById.get(litter.sire_dog_id) ?? null : null, dam: litter.dam_dog_id ? dogsById.get(litter.dam_dog_id) ?? null : null, puppies: safePuppies.map((puppy) => { const publicMedia = mediaByPuppy.get(puppy.id) ?? []; return { collarColor: puppy.collar_color || null, publicName: puppy.public_name || null, sex: puppy.sex || null, color: puppy.public_color || null, status: puppy.public_status || null, price: puppy.public_price ?? null, summary: puppy.public_summary || null, primaryPhoto: publicMedia.find((item) => item.type === "photo" && item.isPrimary)?.url ?? publicMedia.find((item) => item.type === "photo")?.url ?? null, developmentSummaries: summariesByPuppy.get(puppy.id) ?? [], publicMedia }; }) }, { headers: cacheHeaders });
}
